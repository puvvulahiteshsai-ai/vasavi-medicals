            ...
            <li key={s.title} className="svc-card flip-card" style={{ '--ic': s.ic, '--ib': s.ib }}>
              <div className="flip-card-inner">
                ...
                <div className="flip-card-back">
                  <div className="svc-card__icon" style={{ background: 'white', color: s.ic || 'var(--blue)', marginBottom: '16px' }}>
                    <i className={`fas ${s.icon}`}></i>
                  </div>
                  <h3 style={{ color: 'var(--blue-dark)' }}>{s.title}</h3>
                  <p style={{ color: 'var(--text-500)', fontSize: '0.85rem' }}>Top quality {s.title.toLowerCase()} available in store.</p>
                </div>
              </div>
            </li>
          ))}
